...
BUILDER = SETTINGS.repo_root / "orchestrator" / "governance" / "worker_roster_builder.py"
...
def select_worker(
    ...
    approve_overtime: bool = False,
    reserve_percent: float = 20.0,
    refresh: bool = False,
) -> dict[str, Any]:
    if context_tokens <= 0 and intent_text:
        context_tokens = max(1, len(intent_text.split()))
    return select(job_class, context_tokens, approve_overtime, reserve_percent, refresh)
